...
class Pool {
    ...
    async changeManager(address, name) {
        ...
        let changeEvent = receipt.events.find(item => item.event === 'ManagerUpdated')

        if (!changeEvent)
			throw new Error('Manager not updated')
    ...
    async getAssets() {
        return (await this.contract.getSupportedAssets()).map(item => ethers.utils.parseBytes32String(item))
    }
    ...
    async getWaitingPeriods() {
        ...
        result[0].map((item, index) => {
            ...
            periods[ethers.utils.parseBytes32String(item)] = result[1][index].toNumber()
    ...
    async withdraw(amount, forfeit = false) {
        ...
        if (forfeit)
            ...
            tx = await this.contract.withdraw(amount)
    ...
    async exchange(sourceKey, sourceAmount, destinationKey) {
        ...
        let tx = await this.contract.exchange(sourceKey, sourceAmount, destinationKey)
